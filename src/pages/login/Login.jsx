...
const axiosPublic=useAxiosPublic();
   ...
   const handleGoogle = () => {
    googleSignIn()
      .then((result) => {
        ...
        swal("成功!", "您已成功登录", "success");
        const destination = location.state ? location.state : '/';
        console.log('我的目的地:',destination);
   ...
   登<span className='text-pink-600'>录</span>
  </h4>
  ...
  <div className='w-6/12 h-8/12 rounded-lg relative'>
            <div className="relative">
              ...
              <p className="absolute top-1/2 left-1/2 transform -translate-x-1/2 text-center text-white -translate-y-1/2 text-sm lg:text-3xl  ">
                欢迎来到 <span className='lg:text-4xl text-xl text-pink-600 px-4 py-2 rounded-md font-bold'>FourPows</span> 宠物领养中心。<br />登录以探索我们的网站。
              </p>
            </div>
          </div>
 <div className="  mx-3 px-3 bg-white-200 border-2 border-pink-500 rounded-lg" >
  ...
  <form className="mt-8 mb-2 w-full max-w-screen-lg sm:w-96 px-3 py-10 shadow-md rounded-md" onSubmit={handleLogin}>
    ...
    <p className=' text-sm mt-10 text-black'>忘记密码？</p>
    <button
  ...
  <p className='text-center mt-2'>或</p>
    <button
    ...
    >
    <FcGoogle className='hidden lg:flex absolute w-6 h-6 left-20 -mt-1'></FcGoogle> 使用 Google 登录
    </button>
      <p className="p-2 text-sm md:p-5">还没有账号？请 <Link to='/register' className=" text-pink-600 font-bold">注册</Link></p>
    </div>
  </div>
  </div>
             </div>
