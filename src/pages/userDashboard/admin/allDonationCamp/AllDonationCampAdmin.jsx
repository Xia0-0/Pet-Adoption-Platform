...
const AllDonationCampAdmin = () => {
    ...
    useEffect(() => {
      ...
      const deletePet = (_id) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            fetch(`${import.meta.env.VITE_API_BASE_URL}/adddonationcamp/${_id}`, {
              method: 'DELETE',
            })
              .then(response => response.json())
              .then(data => {
      ...
      const handlePause = (pet) => {
        ...
        axiosSecure
          .patch(`${import.meta.env.VITE_API_BASE_URL}/admin/pause/${petId}`)
          .then((res) => {
      ...
      const handleResume= (pet) => {
        ...
        axiosSecure
          .patch(`${import.meta.env.VITE_API_BASE_URL}/admin/resume/${petId}`)
          .then((res) => {
            console.log(res.data);
            if (res.data.modifiedCount === 0) {
              // Revert the local state if the request fails
